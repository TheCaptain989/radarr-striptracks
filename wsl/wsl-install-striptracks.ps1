...
if ($LASTEXITCODE -eq 50) {
    Write-Output "WSL does not appear to be installed.  Run 'wsl --install' first."
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Output "There was an error when attempting to install required Linux packages."
    exit 1
}
